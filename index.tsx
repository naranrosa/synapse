...
interface Doctor {
  ...
  color: string;
  is_admin?: boolean;
  team_id?: number; // Cada médico pertence a uma equipe
}

interface Hospital {
  id: number;
  name: string;
  team_id: number; // Cada hospital pertence a uma equipe
}

interface InsurancePlan {
  id: number;
  name: string;
  team_id: number; // Cada convênio pertence a uma equipe
}
...
interface Surgery {
  ...
  notes: string;
  pre_op_xray_path?: string;
  post_op_xray_path?: string;
  team_id: number; // Cada cirurgia pertence a uma equipe
}
            ...
            <div className="login-box">
                ...
                <form onSubmit={handleLogin}>
                    <div className="form-group">
                        ...
                        <input type="email" id="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="exemplo@med.com"/>
                    </div>
                    <div className="form-group">
    ...
    <header className="app-header">
      ...
      <div className="header-actions">
        <div className="header-search">
            ...
            <input type="search" placeholder="Buscar paciente..." value={searchQuery} onChange={onSearchChange}/>
        </div>
...
const SurgeryModal: React.FC<{
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.patient_name.trim()) { addToast('O nome do paciente é obrigatório.', 'error'); return; }
            ...
            <div className="report-details-grid">
                <div>
                    ...
                    <div className="add-item-form">
                        <input type="text" value={newHospital} onChange={e => setNewHospital(e.target.value)} placeholder="Nome do novo hospital"/>
                ...
                <div>
                    <h3>Convênios</h3>
                    <div className="add-item-form">
                        <input type="text" value={newPlan} onChange={e => setNewPlan(e.target.value)} placeholder="Nome do novo convênio"/>
...
const AdminView: React.FC<{
  ...
  useEffect(() => {
    if (editingUser) {
      setFormState({ name: editingUser.name, email: editingUser.email, password: '', is_admin: !!editingUser.is_admin });
    } else {
    ...
    <div className="admin-container">
      ...
      <div className="admin-form-card">
          ...
          <form onSubmit={handleSubmit} className="admin-form">
              <div className="form-group"><label htmlFor="name_admin">Nome</label><input type="text" id="name_admin" name="name" value={formState.name} onChange={handleInputChange} required /></div>
              <div className="form-group"><label htmlFor="email_admin">Email</label><input type="email" id="email_admin" name="email" value={formState.email} onChange={handleInputChange} required /></div>
              <div className="form-group"><label htmlFor="password_admin">Senha</label><input type="password" id="password_admin" name="password" value={formState.password} onChange={handleInputChange} placeholder={editingUser ? "Deixe em branco para não alterar" : "Senha obrigatória"} /></div>
              <div className="form-group-checkbox"><label><input type="checkbox" name="is_admin" checked={formState.is_admin} onChange={handleInputChange} />É Administrador?</label></div>
...
useEffect(() => {
  ...
  const loadProfile = async (session: any) => {
    try {
      if (session?.user) {
        const { data: profile, error } = await supabase.from('doctors').select('*').eq('id', session.user.id).single();
        if (error || !profile) {
  ...
  const handleLogout = async () => {
    try { await supabase.auth.signOut(); } catch (err) { console.error("Erro ao sair:", err); } finally {
        setLoggedInUser(null); setUsers([]); setSurgeries([]); setHospitals([]); setInsurancePlans([]); setCurrentView('dashboard');
    }
  };

  const handleSaveSurgery = async (formData: any, files: { preOp?: File, postOp?: File }) => {
    if (!loggedInUser?.team_id) {
        addToast('Erro: Equipe do usuário não encontrada.', 'error');
        return;
    }
    try {
        const sanitizeFilename = (name: string) => name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9.\-_]/g, '_').replace(/\s+/g, '_');
        let pre_op_xray_path = formData.pre_op_xray_path;
        let post_op_xray_path = formData.post_op_xray_path;

        if (files.preOp) {
            const filePath = `${loggedInUser.id}/${Date.now()}-${sanitizeFilename(files.preOp.name)}`;
            const { error } = await supabase.storage.from('xrays').upload(filePath, files.preOp);
            if (error) throw error;
            pre_op_xray_path = filePath;
        }
        if (files.postOp) {
            const filePath = `${loggedInUser.id}/${Date.now()}-${sanitizeFilename(files.postOp.name)}`;
            const { error } = await supabase.storage.from('xrays').upload(filePath, files.postOp);
            if (error) throw error;
        ...
        addToast(surgeryToEdit ? 'Cirurgia atualizada!' : 'Solicitação salva!', 'success');
  ...
  const handleDeleteHospital = async (id: number) => {
      ...
      if(error) addToast(`Erro: ${error.message}`, 'error'); else { setHospitals(prev => prev.filter(h => h.id !== id)); addToast('Hospital excluído!', 'success'); }
  };
  ...
  const handleDeletePlan = async (id: number) => {
      ...
      if(error) addToast(`Erro: ${error.message}`, 'error'); else { setInsurancePlans(prev => prev.filter(p => p.id !== id)); addToast('Convênio excluído!', 'success'); }
  };
  ...
  const handleUpdateSurgeryStatus = async (surgeryId: number, status: 'Realizada' | 'Cancelada') => {
    ...
    if (error) addToast(`Erro: ${error.message}`, 'error'); else addToast('Status atualizado!', 'success');
  };
  ...
  const handleSurgeryDrop = async (surgeryId: number, newDate: Date) => {
    ...
    const { error } = await supabase.from('surgeries').update({ date_time: newDateTime.toISOString() }).eq('id', surgeryId);
    if (error) addToast(`Erro: ${error.message}`, 'error'); else addToast('Cirurgia reagendada!', 'success');
  };

  const handleUsersChange = useCallback(async () => {
    const { data, error } = await supabase.from('doctors').select('*');
    if (error) addToast(`Erro: ${error.message}`, 'error'); else setUsers(data || []);
  }, [addToast]);
  ...
  const handleNavigateWithFilter = (view: any, filters: any) => { setInitialFilters(filters); setCurrentView(view); };
  const handleNavigate = (view: any) => { setInitialFilters(null); setCurrentView(view); }
  ...
  if (appLoading) {
     return (<div className="loading-fullscreen"><div className="logo-container"><span className="material-symbols-outlined">health_and_safety</span></div><h2>Carregando agenda...</h2></div>);
  }
  if (!loggedInUser) {
    return <LoginView />;
  }
